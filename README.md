...
| `listen` | Block until the human **messages you** from the app; prints them, exits `0`. One-shot — loop it. `--follow` instead holds the session open and keeps printing messages as they arrive (use it only when you intend to sit and wait). A read message is DELIVERED (gray ✓✓), **not** done — `ack` it after the work (`--ack-on-read` for immediate consume). Its stdout is a **documented shape** — see [The listen stdout contract](#the-listen-stdout-contract) — and `--ndjson` switches it to one object per line. **`--exec '<handler>'` runs the round for you:** the batch JSON on the handler's stdin (`{"messages":[…],"continuity":[…]}`, ONE invocation) and **the handler's exit code decides the ack** — `0` acks the batch's exact ids with its `pidge-summary:` line as the note; anything else (non-zero, spawn error, `--handler-timeout`) acks **nothing**, prints `{"type":"handler_failed","exit":N,"reason":…,"ids":[…]}` on **stdout** and exits `2`; an exit `0` whose **ack fails** prints `{"type":"ack_failed","ids":[…]}` and exits `2` too (the work happened, the server doesn't know it). The handler's stdout is teed through and the lease is renewed every 60 s while it runs; `--exec` refuses to combine with `--follow`/`--ndjson`/`--ack-on-read` (exit `1`). ⚠️ `listen` **consumes**, and it now **HOLDS this channel's consumer lock** for its whole run: a second `listen` — or a `bridge` — is refused (exit `2`) and pointed at `catchup`; a crashed listener's lock is reclaimed by the next one (pid-checked). **Gate hygiene** (same rule as the bridge, server manifest ≥ v83): a `notification_reply` marked `ref.gated` — a Face-ID gate outcome — is acked here and **never surfaced as a row** (a gate answer is not a command; the asker already heard it), so a round of nothing but gate answers is an empty round. Exits: `0` delivered/handled · `3` empty round · `4` empty AND no healthy round-trip · `2` error, refusal or failed handler. |
| `online` | Sugar for **`pidge listen --all`** — the stay-online command, one word, so a pasted prompt can just say *"stay online: pidge online"*. **As yourself (0.54):** `pidge online --follow --ndjson --timeout 0` is the session-length WATCH — one message per stdout line, NO deadline — for a harness that streams a process's stdout to the agent as events (Claude Code: `Monitor({command:'pidge online --follow --ndjson --timeout 0', persistent:true})`): the human's messages reach the session they are talking to, and when that session ends they see the agent OFFLINE — honest, like a chat. A bridge holding the channel yields to the watch and takes over again when it ends. Without such a watch: one round per invocation as a background task your harness **tracks** (never a loose shell `&`); it blocks, exits with the batch (3 = nothing), you handle → `ack` → **relaunch** — knowing the harness may kill it when the turn ends (you lose time, never a message). stdout stays parseable JSON; the nudges on stderr (after `setup`/`hello`/`doctor` with no live consumer, on an empty round, after an agent-driven `ack`) say exactly this. |
| `bridge --exec '<handler>'` | The **24/7 supervisor** — and, be clear, **a STAND-IN: another agent answering in the human's chat while nobody is there.** Many humans do not want that ("if you are gone, show me offline"): run it only when yours asked for one. Long-polls the channel (`--all`); your handler runs **once per batch** with the batch JSON on stdin (also at `$PIDGE_BATCH_FILE`; `{"messages":[…]}` + `history_hint:true` on the first batch since boot); handler exit `0` ⇒ ack of the batch's **exact ids** · non-zero ⇒ **not acked** (the server's **~10-min** visibility lease re-serves — make the handler **idempotent**). One run is capped by `--handler-timeout` (default 30 min: SIGTERM, SIGKILL 5s later, counts as a failed batch), with a stderr heartbeat every 5 min while the handler runs **and an automatic lease/presence renew every 60 s**. **The handoff (0.54):** a bridge started while a live `listen` holds the channel STANDS BY (it no longer dies and flap-restarts); a `listen` started while the bridge holds it asks the bridge to YIELD (SIGUSR2 — sent only to a lock that names itself a bridge) and the bridge hands over after its current cycle (a running handler finishes and is acked first), then takes the channel back when that listen exits. So the live agent always wins the channel and the stand-in covers the gaps. **One instance per channel**: a PID-checked lockfile refuses a second bridge (a stale lock from a crash is recovered). 401 / broken channel → narrated + **local alert** + long **jittered** backoff — never a silent death, never a blind hot loop. SIGTERM/SIGINT are clean: in-flight batch NOT acked, lock released, exit `0`. Deliberately DUMB: no local queue — durability is the server's ack/lease. **Attribution:** the handler prints a final marker line — `pidge-summary: <one sentence>` — and the bridge acks with that summary so `catchup` shows *"handled by X: <summary>"*; no marker ⇒ acked without one (never invented). **Gate hygiene** (server manifest ≥ v83): a `notification_reply` marked `ref.gated` is acked without spawning a handler. |
| `bridge install [--handler claude\|codex\|gemini \| --exec '<handler>'] [--enable]` · `bridge status` · `bridge uninstall` | **Opt-in stand-in, one command (0.54).** Writes the launchd (macOS) / systemd user (Linux) service that runs the bridge **from THIS project** (`WorkingDirectory` — a project-scoped key resolves by cwd) with the PATH you have now (node's own dir in front), `Restart=on-failure`, and declares `listen_mode=external_daemon` (advisory). **`--handler`** (default: the first of `claude`/`codex`/`gemini` on PATH) GENERATES the handler — `bridge-handler.sh` + an editable `bridge-prompt.md` + a `pidge` shim, in the identity's config dir (`.bak` on rewrite): batch to a file, the prompt through stdin (a model CLI's prompt *argument* is unreliable once stdin is a pipe), `claude` as ONE resumed session across batches (`--session-id` then `--resume`; a failed resumed run drops the id so the next batch starts fresh), a system-only batch (a selftest nonce, a contract change) answered WITHOUT calling the model after checking the model CLI resolves under the daemon (the PATH failure that kills daemons is caught by the selftest), a generic `pidge-summary:` only when the model printed none, and the prompt that tells the model it is the stand-in, to reply THROUGH `pidge message`, never to run `setup`/`listen`/`bridge`/`ack`. Running from the npx cache ⇒ the service pins `npx -y pidge-cli@<this version>` instead of the cache path (no `npm i -g` detour). **`--enable`** runs the enable commands, waits for a live consumer, then runs a **selftest — exit 0 only on PASS** (JSON `{ok, enabled, selftest}`); the template **never embeds the key** (a key that lives only in your shell env is warned about). `bridge status` = the measured verdict (service · local lock · server consumers, exit 0 ONLINE / 3 OFFLINE); `bridge uninstall` stops + removes the service and re-declares `turn_based`. |
...
| `selftest [--window N]` | Prove your listener works by ROUND-TRIP — fire a nonce, then watch READ-ONLY for something ELSE (your watch, a `listen`, a bridge) to pick it up and ack it inside the window (PASS exit `0` / FAIL exit `2` with the likely cause: nothing listening / a deaf or slow consumer / transport). `--window` up to **600 s** (0.54; server manifest ≥ v125 — older servers clamp to 120 and the CLI watches the window the server granted): a consumer whose handler runs a model on every batch acks only when the model returns. Run it after wiring the watch + whenever sends seem to go unheard. |
...
| `presence` · `hook install` / `uninstall` | **`presence`** prints ONE line: *"OFFLINE — nobody is listening; start the watch: Monitor(…)"* or *"listening — X holds the queue; read with `catchup`, never listen"* (exit 0). Built for Claude Code's **SessionStart** hook, which fires on startup, resume, `/clear` and `/compact` — the moments an agent born from zero forgets it was supposed to be reachable. `setup` installs the hook into the project's `.claude/settings.json` under Claude Code (`--no-hook` skips); `hook install`/`uninstall` manage it by hand (tagged entry, other hooks untouched). |
