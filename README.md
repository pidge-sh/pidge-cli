...
| `terminal [--name X] [-- CMD…]` / `terminal attach <name>` | **Pidge Terminal** — mirror a LIVE tmux session to the human's phone/Mac (the app's Terminals tab), keystrokes coming back; the agent running *inside* tmux needs no integration and never knows. The bare form **creates** a detached tmux session (running `CMD`, else the default shell) and mirrors it — the human can also `tmux attach -t <name>` locally, both at once; `attach` mirrors a session that already exists. This process is only the **mirror**: Ctrl-C stops mirroring (the tmux session keeps running, resume with `attach`); when the session itself dies the mirror marks it ended and exits `0`. **Sealed-only, no opt-out**: every frame is end-to-end encrypted with the channel key (the server relays ciphertext it cannot read) — needs an E2E channel **and** `PIDGE_SECRET`, else it refuses (exit `2`) with the fix. Session name/status are the only clear metadata — keep secrets out of `--name`. Requires `tmux` (macOS: `brew install tmux`) and Node ≥22. **Pro feature**: a non-Pro account gets a typed `402` with a message to relay (don't retry). stdout prints one JSON line (`{ok, public_id, name, epoch}`). **Channel link (advisory, server ≥ v94)**: `--link ID` tags the session with the channel whose agent runs inside — a provenance chip in the Terminals tab; `--no-link` sends an explicit null that **clears** a stored link (an omitted link keeps it). With neither flag, a session whose cwd lives in a git project holding a project-scoped pidge env is linked to that project's channel automatically — loudly (the note names the channel), conservatively (any doubt ⇒ no link), never fatally (a refused *inferred* link registers without it; a refused explicit `--link` is a loud `422`, exit `2`). The link never changes delivery, auth or relay behavior. |
| `terminal host [--install [--machine-channel]]` | The **always-on daemon** that makes the Terminals tab self-service (one per channel, PID-checked lock). It registers a **control lane** (where the app reads the sessions list + spawn profiles, sealed like everything else), keeps every `tmux ls` session registered as it appears/disappears, **lazy-attaches** a session only while someone is watching (join → attach + repaint; last leave → stand down after ~30 s — zero viewers, zero traffic), and **spawns strictly from the whitelist** in `~/.config/pidge/terminal.toml` (`[[profile]]` with `name`/`cwd`/`cmd`) — a viewer can only *name* a profile; command lines never come from the wire (they live on your machine). One WebSocket for everything (control + N sessions). `--install` writes the launchd (macOS) / systemd user (Linux) template (restart-on-failure; review, then enable with the printed command — the key is **never** embedded). Add **`--machine-channel`** and the installer auto-creates a **hidden** machine channel (`🖥️ <hostname>`, server ≥ v94 — an older server is refused loudly *before* anything is created) with your existing key, or **reuses** the one a previous install minted (never a duplicate); its key is stored in the daemon's own scope (`~/.config/pidge/agents/terminal-host/env`) — the shared and project envs are never touched — and the template pins `PIDGE_AGENT=terminal-host` so the daemon resolves it. The channel is born non-E2E: the installer prints the sealed-only next steps (enable E2E in the app; `PIDGE_SECRET` lands next to the token). Clean shutdown ends only the control row; tmux sessions are never touched. |
