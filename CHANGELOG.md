...
- **Per-request identity on EVERY call.** The CLI sends `X-Pidge-Fingerprint` + `X-Pidge-Label`
  ...
  ANONYMOUS (a diagnosis must not mint a phantom consumer). Advisory, never auth (#181 posture).
...
- **cli#74: `catchup` stops re-downloading sealed attachments every session.** `--digest` implies
