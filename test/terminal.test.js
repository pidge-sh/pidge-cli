...
test('normalize: slash-command plumbing never becomes a user bubble — but a mere MENTION is kept (QA r6-4)', () => {
  const user = (content, uuid = 'u-slash') => ({ type: 'user', uuid, timestamp: 't', message: { role: 'user', content } });

  // The real shapes, verbatim from live transcripts (the /clear aftermath the
  // phone rendered as three purple bubbles of XML attributed to the human).
  const plumbing = [
  ...
  for (const content of plumbing) {
    assert.deepEqual(adapter.normalize(user(content)), [], `plumbing must be dropped: ${content.slice(0, 40)}`);
    // The same content arriving as a text BLOCK is plumbing too.
    assert.deepEqual(adapter.normalize(user([{ type: 'text', text: content }])), []);
  }

  // Conservative: a legitimate message that only MENTIONS the tags, or that
  // carries prose alongside a span, is NOT structurally plumbing — kept whole.
  const kept = [
    'why does the transcript show <command-name>/clear</command-name> to me?',
    '<command-name>/clear</command-name> — and then it broke, see above',
    '<command-unrelated>hello</command-unrelated>',
    '<command-name>unclosed tag soup',
  ];
  for (const content of kept) {
    const items = adapter.normalize(user(content));
    assert.equal(items.length, 1, `a real user message must survive: ${content.slice(0, 40)}`);
    assert.equal(items[0].preview, content);
    assert.equal(items[0].role, 'user');
  }

  // And the assistant echoing the same tags is never filtered (user-role only).
  const echo = { type: 'assistant', uuid: 'a-slash', timestamp: 't', message: { role: 'assistant', content: [{ type: 'text', text: '<command-name>/clear</command-name>' }] } };
